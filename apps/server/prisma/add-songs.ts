...
function cleanSongName(filename: string): string {
  let name = filename
    ...
    .replace(/_([a-z0-9]{6,8})$/, '')
    .replace(/-/g, ' ')
    .replace(/_/g, ' ')
    .replace(/MassTamilan\.(com|dev|fm|so|org)/gi, '')
    .replace(/KoshalWorld\.Com/gi, '')
    .replace(/PenduJatt\.Com\.Se/gi, '')
    .replace(/Saurav_Srisan_Rathinamo_Official_Lyric_Video/gi, 'Rathinamo')
    .replace(/spotdown\.org/gi, '')
    .replace(/\s+/g, ' ')
    .trim();
...
async function main() {
  console.log('Adding real songs to database...');
  ...
  console.log(`Total URLs: ${urls.length}, Unique: ${uniqueUrls.length}`);

  const artists = await prisma.artist.findMany();
  if (artists.length === 0) {
    console.log('No artists found. Run seed first.');
    return;
  }
  ...
  console.log(`Existing songs with audioUrl: ${existingUrls.size}`);
  ...
  for (let i = 0; i < uniqueUrls.length; i++) {
    ...
    const songName = cleanSongName(filename);

    if (!songName) {
      console.log(`Skipping (empty name): ${filename}`);
      skipped++;
      continue;
    }
    ...
    try {
      const existing = await prisma.song.findFirst({ where: { audioUrl: url } });
      ...
      await prisma.song.create({
        data: {
          ...
          audioUrl: url,
          coverImage: album.coverImage || '/placeholder-album.svg',
          artistId: artist.id,
          albumId: album.id,
          genreId,
          isExplicit: false,
      ...
      added++;
      if (added % 20 === 0) console.log(`Added ${added} songs so far...`);
    } catch (err: any) {
      if (err.code === 'P2002') {
        skipped++;
      } else {
        console.error(`Error adding "${songName}": ${err.message}`);
        skipped++;
      }
    }
  }
  ...
  const songsWithAudio = await prisma.song.count({ where: { audioUrl: { not: null } } });
  console.log(`Total songs in DB: ${totalSongs}, with audio: ${songsWithAudio}`);
}
